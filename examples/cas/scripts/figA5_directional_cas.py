    ...
    for _, grp in sub.groupby("horizon"):
        ...
        try:
            _, det = cluster_aware_severity_score(
                y, yp, window_size=h, kernel=KERNEL,
                lambda_=LAMBDA_, gamma=GAMMA_, return_details=True)
        except Exception:
            continue
...
for domain in DOMAIN_ORDER:
    ...
    for model in MODEL_ORDER:
        ...
        if np.isfinite(n_o):
            print(f"  {domain}/{model}: over={n_o:.1f}({s_o:.4f})  "
...
for di, domain in enumerate(DOMAIN_ORDER):
    ...
    if len(x) > 0:
        b_over  = ax.bar(x, over_pcts,  BW, color=col_over,  alpha=0.88,
                         edgecolor="white", lw=0.8, label="Upper (over $q_{90}$)")
        b_under = ax.bar(x, under_pcts, BW, bottom=over_pcts, color=col_under,
                         alpha=0.88, edgecolor="white", lw=0.8, hatch="...",
                         label="Lower (under $q_{10}$)")

        # Annotate percentages inside the bars
        for i, (po, pu) in enumerate(zip(over_pcts, under_pcts)):
            if po > 5:
                ax.text(x[i], po / 2, f"{po:.0f}%",
                        ha="center", va="center", fontsize=9, color="white",
                        fontweight="bold")
            if pu > 5:
                ax.text(x[i], po + pu / 2, f"{pu:.0f}%",
                        ha="center", va="center", fontsize=9, color=darken(col, 0.5),
                        fontweight="bold")
...
for di, domain in enumerate(DOMAIN_ORDER):
    ...
    for mi, model in enumerate(MODEL_ORDER):
        ...
        b1 = ax.bar(x0,         s_o, BW, color=col_over,  alpha=0.88,
                    edgecolor="white", lw=0.8,
                    label="Upper severity" if mi == 0 else "")
        b2 = ax.bar(x0 + BW + GAP, s_u, BW, color=col_under, alpha=0.88,
