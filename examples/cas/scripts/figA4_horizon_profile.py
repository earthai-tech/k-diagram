...
fig.subplots_adjust(
    left=0.07, right=0.88, top=0.90, bottom=0.09, hspace=0.52, wspace=0.33
)
...
for di, domain in enumerate(DOMAIN_ORDER):
    ...
    for mi in range(len(MODEL_ORDER)):
        for hi_idx in range(len(horizons)):
            ...
            norm_v = (
                (v - vmin) / max(vmax - vmin, 1e-9) if np.isfinite(v) else 0
            )
...
sm = plt.cm.ScalarMappable(
    cmap=CAS_CMAP, norm=plt.Normalize(vmin=vmin, vmax=vmax)
)
sm.set_array([])
