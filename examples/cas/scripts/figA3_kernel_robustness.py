...
for domain in DOMAIN_ORDER:
    ...
    for model in MODEL_ORDER:
        ...
        print(f"  {domain}/{model}: tri={results[domain][model]['triangular']:.4f}"
              f"  box={results[domain][model]['box']:.4f}")
    point_data[domain] = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()
...
for di, domain in enumerate(DOMAIN_ORDER):
    ...
    ax.plot([vmin, vmax], [vmin, vmax], color="0.55", lw=0.9, ls="--",
            label="$y = x$", zorder=1)
    ...
    for model in MODEL_ORDER:
        ...
        ax.scatter(sub["cas_tri"], sub["cas_box"],
                   s=65, color=lc, marker=MODEL_MARK[model],
...
for di, domain in enumerate(DOMAIN_ORDER):
    ...
    for mi, model in enumerate(MODEL_ORDER):
        ...
        b1 = ax.bar(x0,         tri, BW, color=lc_tri, alpha=0.88,
                    edgecolor="white", lw=0.8,
                    label="Triangular" if mi == 0 else "")
        b2 = ax.bar(x0 + BW + GAP, box, BW, color=lc_box, alpha=0.88,
    ...
    bar_vals = [bar_data[domain][m].get(k, np.nan)
                for m in MODEL_ORDER for k in KERNELS
