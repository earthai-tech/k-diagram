...
try:
    from results_config import (
        ...
        OUTDIR, PRED_HYDRO, PRED_SUBS, PRED_WIND,
    ...
    def enforce_non_crossing(q10, q50, q90):
        q50c = np.maximum(q50, q10); return q10, q50c, np.maximum(q90, q50c)
...
def compute_AB(df, model, h=H_DEFAULT, kernel=KERNEL, gamma=GAMMA_):
    ...
    for _, grp in sub.groupby("horizon"):
        ...
        try:
            A = cluster_aware_severity_score(y, yp, window_size=h, kernel=kernel,
                                              lambda_=0.0, gamma=gamma)
            # CAS at λ=1 → A_j + B_j
            AB = cluster_aware_severity_score(y, yp, window_size=h, kernel=kernel,
                                               lambda_=1.0, gamma=gamma)
...
for di, domain in enumerate(DOMAIN_ORDER):
    ...
    ax.axvspan(*LAM_PRACTICAL, color=col, alpha=0.07, zorder=0,
    ...
    for model in MODEL_ORDER:
        ...
        ln, = ax.plot(LAM_RANGE, cas_lam,
                      ls=MODEL_STYLE[model], lw=2.0, color=lc,
    ...
    for i in range(len(models_plotted)):
        for j in range(i + 1, len(models_plotted)):
            ...
            if lam_x is not None and LAM_RANGE[0] <= lam_x <= LAM_RANGE[-1]:
                cas_x = Ai + lam_x * Bi
                ax.scatter(lam_x, cas_x, s=80, zorder=6,
                           color="0.2", marker="X", edgecolors="white", lw=0.8)
                ax.annotate(
                    f"$\\lambda^*={lam_x:.2f}$",
                    xy=(lam_x, cas_x),
    ...
    cas_at_default = [AB[domain][m][0] + LAM_DEFAULT * AB[domain][m][1]
                      for m in MODEL_ORDER
                      if np.isfinite(AB[domain][m][0]) and np.isfinite(AB[domain][m][1])
                      and AB[domain][m][0] + LAM_DEFAULT * AB[domain][m][1] > 0]
    use_log_y = len(cas_at_default) > 1 and max(cas_at_default) / min(cas_at_default) > 10
    if use_log_y:
        ax.set_yscale("log")
        ax.yaxis.set_major_formatter(mtick.LogFormatterSciNotation(labelOnlyBase=False))
    else:
...
for domain in DOMAIN_ORDER:
    ...
    for model in MODEL_ORDER:
        ...
        if not (np.isfinite(A) and np.isfinite(B)):
            continue
        bot_ax.scatter(A, B, s=110,
                       color=col, marker=MODEL_MARK[model],
                       edgecolors="white", lw=0.9, zorder=4,
                       label=f"{DOMAIN_LABEL[domain]} / {MODEL_LABEL[model]}")
        bot_ax.annotate(
            MODEL_LABEL[model],
            xy=(A, B), xytext=(A, B + max(0.0002, B * 0.04)),
            ha="center", va="bottom", fontsize=7.5, color=col,
        )

# Use log scale when A or B values span more than one order of magnitude
all_A = [AB[d][m][0] for d in DOMAIN_ORDER for m in MODEL_ORDER
         if np.isfinite(AB[d][m][0]) and AB[d][m][0] > 0]
all_B = [AB[d][m][1] for d in DOMAIN_ORDER for m in MODEL_ORDER
...
bot_ax.set_ylabel("Density slope  $B_j$  (gain per unit $\\lambda$)", fontsize=10)
bot_ax.set_title(
...
legend_handles = [
    Patch(facecolor=DOMAIN_COLOR[d], edgecolor="white", alpha=0.85,
          label=DOMAIN_LABEL[d])
    for d in DOMAIN_ORDER
] + [
    Line2D([0], [0], marker=MODEL_MARK[m], color="0.3", lw=0,
           ms=7, label=MODEL_LABEL[m])
    for m in MODEL_ORDER
]
bot_ax.legend(handles=legend_handles, ncol=6, fontsize=9,
