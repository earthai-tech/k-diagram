    ...
    for _, grp in sub.groupby("horizon"):
        ...
        try:
            s = cluster_aware_severity_score(
                y, yp, window_size=h, kernel=KERNEL, lambda_=LAMBDA_, gamma=GAMMA_)
            if np.isfinite(s):
                scores.append(s)
        except Exception:
            pass
...
for ax, domain in zip(axes, DOMAIN_ORDER):
    ...
    ax.axvline(H_DEFAULT, color="0.55", lw=0.9, ls=":",
               label=f"Default $h={H_DEFAULT}$", zorder=1)
    ...
    for model in MODEL_ORDER:
        ...
        ax.plot(x, cas_vals,
                ...
                marker=MODEL_MARK[model], ms=6.5,
                color=lc, alpha=0.92,
    ...
    all_finite = [v for m in MODEL_ORDER
                  for v in records[domain][m] if np.isfinite(v) and v > 0]
