...
import numpy as np
import pandas as pd
from kdiagram.metrics import cluster_aware_severity_score as cas_score
...
def crps_from_quantiles(y, qmap, *, taus):
    ...
    w = [0.5 * (taus_ext[i+1] - taus_ext[i-1]) for i in range(1, len(taus_ext)-1)]
    w = np.asarray(w, float)
    losses = [_pinball(y - np.asarray(qmap.get(t, np.full_like(y, np.nan)), float), t) for t in taus]
    L = np.vstack(losses).T
    return 2.0 * np.nansum(L * w, axis=1)


def winkler(y, ql, qu, alpha=ALPHA):
    y, ql, qu = np.asarray(y,float), np.asarray(ql,float), np.asarray(qu,float)
    w = qu - ql
    return w + (2/alpha)*(ql-y)*(y<ql) + (2/alpha)*(y-qu)*(y>qu)


def covg(y, ql, qu):
    return float(np.nanmean((np.asarray(y,float) >= ql) & (np.asarray(y,float) <= qu)))


def compute_cas(y, ql, qu, sort_by):
    y_pred = np.c_[ql, qu]
    return float(cas_score(y, y_pred, window_size=CAS_WINDOW,
                           sort_by=np.asarray(sort_by), nan_policy="omit"))


def metrics_for(df_model):
    """Compute scalar metrics for one (model, horizon) block."""
    rows = []
    for (domain, model, horizon), dd in df_model.groupby(["domain","model","horizon"]):
        cv   = covg(dd["y"], dd["q10"], dd["q90"])
        d_cv = float(cv - (1 - ALPHA))
        wk   = float(np.nanmean(winkler(dd["y"], dd["q10"], dd["q90"])))
        crps = float(np.nanmean(crps_from_quantiles(
            dd["y"].values,
            {0.10: dd["q10"].values, 0.50: dd["q50"].values, 0.90: dd["q90"].values},
            taus=[0.10, 0.50, 0.90])))
        cas_vals = []
        for _, g in dd.groupby("series_id", sort=False):
            mask = np.isfinite(g["y"]) & np.isfinite(g["q10"]) & np.isfinite(g["q90"])
            if mask.sum() == 0:
                continue
            sv = g.loc[mask,"t"].values.astype("datetime64[ns]").astype("int64")
            c = compute_cas(g.loc[mask,"y"].values,
                            g.loc[mask,"q10"].values,
                            g.loc[mask,"q90"].values, sv)
            if np.isfinite(c):
                cas_vals.append(c)
        rows.append(dict(domain=domain, model=model, horizon=horizon,
                         n=int(len(dd)), coverage=cv, delta_cov=d_cv,
                         winkler=wk, crps=crps,
...
merged = merged.sort_values(["model","horizon","series_id","t"]).reset_index(drop=True)
...
all_mets = []
for dom, fp in domain_files.items():
    df_d = pd.read_csv(fp)
