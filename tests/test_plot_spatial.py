...
def uncertainty_df():
    ...
    return pd.DataFrame({"lon": lons, "lat": lats, "y": y, "q10": q10, "q90": q90})
...
class TestPlotSpatialScatter:
    ...
    def test_size_col_bubbles(self, scatter_df):
        ax = plot_spatial_scatter(scatter_df, "lon", "lat", "cas",
                                  size_col="width", size_range=(10, 300))
        assert isinstance(ax, Axes)
    ...
    def test_annotation_col(self, scatter_df):
        ax = plot_spatial_scatter(scatter_df, "lon", "lat", "cas",
                                  annotate=True, annotation_col="label")
        assert isinstance(ax, Axes)
    ...
    def test_savefig_creates_file(self, scatter_df, tmp_path):
        ...
        plot_spatial_scatter(scatter_df, "lon", "lat", "cas",
                             savefig=str(out))
        assert out.exists()
    ...
    def test_all_nan_metric_returns_none(self):
        ...
        with pytest.warns(UserWarning, match="no data remains after dropping NaNs"):
    ...
    def test_empty_dataframe_raises_or_warns(self):
        df = pd.DataFrame({"lon": pd.Series([], dtype=float),
    ...
    def test_existing_ax_reused(self, scatter_df):
        ...
        returned_ax = plot_spatial_scatter(scatter_df, "lon", "lat", "cas",
                                           ax=existing_ax)
...
class TestPlotSpatialHeatmap:
    ...
    def test_no_scatter_overlay(self, scatter_df):
        ...
        path_colls = [c for c in ax.collections
    ...
    def test_savefig_creates_file(self, scatter_df, tmp_path):
        ...
        plot_spatial_heatmap(scatter_df, "lon", "lat", "cas",
                             savefig=str(out))
        assert out.exists()
    ...
    def test_existing_ax_reused(self, scatter_df):
        fig, existing_ax = plt.subplots()
        returned_ax = plot_spatial_heatmap(scatter_df, "lon", "lat", "cas",
                                           ax=existing_ax)
        assert returned_ax is existing_ax
...
class TestPlotSpatialCoverage:
    ...
    def test_savefig_creates_file(self, coverage_df, tmp_path):
        ...
        plot_spatial_coverage(coverage_df, "lon", "lat", "coverage",
                              savefig=str(out))
        assert out.exists()
...
class TestPlotSpatialComparison:
    ...
    def test_custom_vmin_vmax_with_shared_scale(self, comparison_df):
        axs = plot_spatial_comparison(
            comparison_df, "lon", "lat",
            ["cas_qar", "cas_qgbm", "cas_xtft"],
            shared_scale=True, vmin=0.0, vmax=1.0)
        for ax in axs:
...
class TestPlotSpatialOrdering:
    ...
    def test_arrow_step(self, polar_df):
        ax = plot_spatial_ordering(polar_df, "lon", "lat", show_arrows=True, arrow_step=10)
        assert isinstance(ax, Axes)
    ...
    def test_returns_none_on_missing_col(self, polar_df):
        with pytest.raises(Exception):
            plot_spatial_ordering(polar_df, "lon", "lat", order_col="nonexistent")
    ...
    def test_savefig(self, polar_df, tmp_path):
        ...
        ax = plot_spatial_ordering(polar_df, "lon", "lat", savefig=out)
        import os
...
class TestPlotPolarFromSpatial:
    ...
    def test_existing_polar_ax(self, polar_df):
        ...
        result = plot_polar_from_spatial(polar_df, "lon", "lat", "width", ax=ax)
        assert result is ax
...
class TestPlotPairedSpatialPolar:
    ...
    def test_savefig(self, polar_df, tmp_path):
        ...
        plot_paired_spatial_polar(polar_df, "lon", "lat", "width",
                                  savefig=out)
        import os
        assert os.path.exists(out)
