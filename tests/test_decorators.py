...
def test_check_non_emptiness_no_parens_raise():
    with pytest.raises(
        ValueError, match="Argument 'first positional argument' is empty"
    ):
        _iden_first_arg([])
...
def test_SaveFile_series_to_frame_and_default_ext(tmp_path, always_catch_warnings):
    ...
    assert (tmp_path / "noext").with_suffix(".csv").exists() or (
        tmp_path / "noext"
    ).exists()
...
def test_SaveFile_bad_index_and_non_df_element_warns(tmp_path, always_catch_warnings):
    ...
    assert any(
        "is not a DataFrame; saving skipped" in str(w.message)
        for w in always_catch_warnings
    )


def test_SaveFile_unsupported_ext_and_missing_ext_warning(
...
def test_SaveFile_write_kws_filtered_and_writer_exception(
    tmp_path, monkeypatch, always_catch_warnings
):
    ...
    mk(
        savefile=str(tmp_path / "will_fail.csv"),
        write_kws={"index": False, "not_a_param": 1},
    )
    assert any(
        "Failed to save the DataFrame: boom" in str(w.message)
        for w in always_catch_warnings
    )
...
def test_save_file_function_based_tuple_and_unsupported(
    tmp_path, always_catch_warnings
):
