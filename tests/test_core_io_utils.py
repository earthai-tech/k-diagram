...
def test_get_valid_kwargs_filters_and_kwargs_passthrough():
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        assert res == {}
        assert any("Unable to inspect callable signature" in str(w.message)
                   for w in rec)


def test_post_process_fill_drop_index_sort():
    df = pd.DataFrame({"k": [2, 1, 3], "x": [1, None, None], "y": [None, 3, 0]})
    ...
    out2 = _post_process(df, index_col=None, sort_index=False,
                         drop_na="any", fillna=None)
    assert out2.shape[0] == 0

    # sanity: 'all' would drop only rows where *all columns* are NA → none here
    out3 = _post_process(df, index_col=None, sort_index=False,
                         drop_na="all", fillna=None)
    assert out3.shape[0] == 3

    # invalid string → warn & keep data shape
    with warnings.catch_warnings(record=True) as rec:
        warnings.simplefilter("always")
        out4 = _post_process(df, index_col=None, sort_index=False,
                             drop_na="invalid", fillna=None)
