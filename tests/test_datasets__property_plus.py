...
def test_get_data_respects_env_and_creates(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
def test_get_data_warns_on_makedirs_failure(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        assert any("Could not create data directory" in str(w.message) for w in rec)
...
def test_download_file_if_package_resource_copy(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    # Create a fake package with a data submodule containing the file
    mod = _make_pkg_with_file(tmp_path, "kdtmp_pkgA", "data", "hello.txt", "hi")
    ...
    got = download_file_if(meta, data_home=str(cache), download_if_missing=False, verbose=False)
...
def test_download_file_if_force_download_success(
    ...
    monkeypatch.setattr(dsprop, "fancier_downloader", fake_downloader, raising=True)

    cache = tmp_path / "cacheB"
    meta = RemoteMetadata(
        file="remote.txt",
        url="https://example.invalid/data/",
        checksum=None, descr_module=None, data_module="kdtmp_pkgB.data",
    )
    got = download_file_if(meta, data_home=str(cache), download_if_missing=True,
...
def test_download_file_if_force_download_raises_on_failure(
    ...
    meta = RemoteMetadata(
        ...
        url="https://example.invalid/data/",
        checksum=None, descr_module=None, data_module="kdtmp_pkgC.data",
    )
    with pytest.raises(RuntimeError, match="Forced download failed"):
        download_file_if(meta, data_home=str(cache), download_if_missing=True,
                         force_download=True, error="raise", verbose=False)
...
def test_download_file_if_download_missing_success_with_string_metadata(
    ...
    monkeypatch.setattr(
        dsprop, "KD_REMOTE_DATA_URL", "https://example.invalid/repo/", raising=True)
    ...
    got = download_file_if("toy.csv", data_home=str(cache), download_if_missing=True, verbose=False)
...
def test_download_file_if_cache_preferred_when_present(
    ...
    mod = _make_pkg_with_file(tmp_path, "kdtmp_pkgE", "data", "dup.txt", "pkg")
    monkeypatch.syspath_prepend(str(tmp_path))
    ...
    meta = RemoteMetadata(
        file="dup.txt", url="https://x/", checksum=None, descr_module=None, data_module=mod)
...
def test_download_file_if_invalid_params_and_types(tmp_path: Path):
    with pytest.raises(ValueError):
        download_file_if("x", data_home=str(
            tmp_path), download_if_missing=True, error="boom")
