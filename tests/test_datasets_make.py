...
def test_make_cyclical_data_bunch_and_frame_shapes():
    n = 24
    s = 2
    bunch = mk.make_cyclical_data(
        n_samples=n, n_series=s, seed=7, as_frame=False
    )
    ...
    df = mk.make_cyclical_data(
        n_samples=n, n_series=s, seed=7, as_frame=True
    )
...
def test_make_cyclical_data_series_and_param_validation_errors():
    ...
    with pytest.raises(
            ValueError, match=r"Length of series_names .* must match"):
        mk.make_cyclical_data(n_samples=10, 
                              n_series=2, series_names=["only_one"])
    # list length mismatch for scalar/list params
    with pytest.raises(ValueError,
                       match=r"Length of 'pred_bias'"):
        mk.make_cyclical_data(n_samples=10, 
                              n_series=3, pred_bias=[0.0, 1.0])
    # wrong type for param
    with pytest.raises(TypeError, 
                       match=r"'pred_noise_factor' must be float or list"):
        mk.make_cyclical_data(
            n_samples=10, n_series=2, pred_noise_factor=(1.0, 1.5))
...
def test_make_cyclical_data_seed_reproducibility():
    ...
    np.testing.assert_allclose(
        a["y_true"].to_numpy(), b["y_true"].to_numpy())
...
def test_make_fingerprint_data_bunch_and_frame_and_validations():
    # basic bunch
    bunch = mk.make_fingerprint_data(
        n_layers=4, n_features=6, seed=3, as_frame=False
    )
    ...
    # as_frame
    df = mk.make_fingerprint_data(
        n_layers=2, n_features=3, seed=4, as_frame=True)
    assert df.shape == (2, 3)

    # validations
    with pytest.raises(ValueError,
                       match="sparsity must be between 0.0 and 1.0"):
        mk.make_fingerprint_data(sparsity=1.5)
    with pytest.raises(ValueError, 
                       match=r"value_range must be a tuple"):
        mk.make_fingerprint_data(value_range=(2.0, 1.0))
    with pytest.raises(ValueError, 
                       match=r"feature_names .* must match"):
        mk.make_fingerprint_data(n_features=3, feature_names=["F1"])
    with pytest.raises(ValueError, 
                       match=r"layer_names .* must match"):
        mk.make_fingerprint_data(
            n_layers=2, layer_names=["A"])
...
def test_make_uncertainty_data_bunch_and_lists_and_nperiods_zero():
    ...
    assert {"location_id", "longitude", 
            "latitude", "elevation"}.issubset(cols)
    assert any(c.endswith("_q0.1") for c in cols)

    # n_periods = 0 returns only features + target
    df0 = mk.make_uncertainty_data(
        n_samples=5, n_periods=0, seed=0, as_frame=True)
    assert set(df0.columns).issuperset(
        {"location_id", "longitude",
         "latitude", "elevation", "value_actual"}
    )
...
def test_make_uncertainty_data_anomalies_outside_interval():
    df = mk.make_uncertainty_data(
        n_samples=80, n_periods=2, 
        anomaly_frac=0.4, seed=123, as_frame=True
    )
...
def test_make_taylor_data_bunch_and_frame_and_warnings():
    # as bunch
    bunch = mk.make_taylor_data(
        n_samples=50, n_models=2, seed=5, as_frame=False)
    ...
    # as frame
    df = mk.make_taylor_data(n_samples=20,
                             n_models=3, seed=6, as_frame=True)
    ...
    with pytest.warns(
            UserWarning,
            match="corr_range limits should ideally be between 0 and 1"):
        _ = mk.make_taylor_data(
            corr_range=(-0.5, 1.5), seed=1)

    # std_range invalid -> warning + defaults
    with pytest.warns(
            UserWarning, match="std_range factors should be non-negative"):
        _ = mk.make_taylor_data(
            std_range=(-1.0, 0.5), seed=2)

    # noise_level zero while correlations < 1 allowed -> ValueError
    with pytest.raises(
            ValueError, match="noise_level cannot be zero"):
        _ = mk.make_taylor_data(
            noise_level=0.0, corr_range=(0.0, 0.9), seed=3)
...
def test_make_multi_model_quantile_data_bunch_and_frame_and_ordering():
    ...
    for name in bunch.model_names:
        ...
        assert all(
            col.startswith(f"{bunch.prefix}_{name}_q") for col in cols)
    ...
    # as_frame
    df2 = mk.make_multi_model_quantile_data(
        n_samples=10, as_frame=True, seed=12)
...
def test_make_multi_model_quantile_data_validations_and_names():
    ...
    with pytest.raises(
            ValueError, match="Length of model_names .* must match"):
        mk.make_multi_model_quantile_data(
            n_models=2, model_names=["A"])
...
def test_make_multi_model_quantile_data_subset_quantiles():
    ...
    bunch = mk.make_multi_model_quantile_data(
        n_samples=15, n_models=1, 
        quantiles=[0.5, 0.9], seed=8, as_frame=False
    )
    cols = [c for c in bunch.frame.columns 
            if c.startswith("pred_Model_A_q")]
    assert set(c.split("_q")[-1] 
               for c in cols) == {"0.5", "0.9"}
