...
def test_make_cyclical_data_series_and_param_validation_errors():
    ...
    with pytest.raises(
        ...
        mk.make_cyclical_data(
            n_samples=10, n_series=2, series_names=["only_one"]
        )
    ...
    with pytest.raises(
        TypeError, match=r"'pred_noise_factor' must be float or list"
    ):
        mk.make_cyclical_data(
...
def test_make_fingerprint_data_bunch_and_frame_and_validations():
    ...
    # validations
    with pytest.raises(
        ValueError, match="sparsity must be between 0.0 and 1.0"
    ):
...
def test_make_uncertainty_data_bunch_and_lists_and_nperiods_zero():
    ...
    assert {"location_id", "longitude", "latitude", "elevation"}.issubset(
        cols
    )
