...
def _grid_visible(ax):
    ...
    return any(gl.get_visible() for gl in ax.get_xgridlines()) or \
           any(gl.get_visible() for gl in ax.get_ygridlines())
...
def test_set_axis_grid_multiple_axes_list():
    ...
    try:
        set_axis_grid(list(axs), show_grid=True, grid_props={"linestyle": ":", "alpha": 0.7})
        fig.canvas.draw()
...
def test_is_valid_kind_with_validation_rejects_raises():
    with pytest.raises(ValueError):
        is_valid_kind("spiderweb", valid_kinds=["scatter", "line", "bar"], error="raise")
...
def test_is_valid_kind_with_validation_rejects_nonraise():
    ...
    out = is_valid_kind("spiderweb", valid_kinds=["scatter", "line", "bar"], error="warn")
