...
# Assume the functions are in this location
...
def test_compute_interval_width_single_pair(wide_forecast_data):
    ...
    expected_width = wide_forecast_data['q90_2023'] - wide_forecast_data['q10_2023']
    pd.testing.assert_series_equal(df['width_q90_2023'], expected_width, check_names=False)
...
def test_calculate_probabilistic_scores_returns_correct_df(probabilistic_data):
    ...
    assert all(col in scores_df.columns for col in ['pit_value', 'sharpness', 'crps'])
...
def test_pivot_forecasts_long_basic(wide_forecast_data):
    ...
    assert all(col in long_df.columns for col in ['horizon', 'q_low', 'q_median', 'q_high'])
