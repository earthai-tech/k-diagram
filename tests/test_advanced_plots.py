...
def horizon_data():
    ...
    df = pd.DataFrame({
        'q10_s1': [1, 2, 3, 4], 'q10_s2': [1.2, 2.3, 3.4, 4.5],
        'q90_s1': [3, 4, 5.5, 7], 'q90_s2': [3.1, 4.2, 5.7, 7.3],
        'q50_s1': [2, 3, 4.2, 5.7], 'q50_s2': [2.1, 3.2, 4.4, 5.9],
    }, index=horizons)
    return df
...
@pytest.mark.parametrize("mask_angle, mask_radius", [(True, False), (False, True), (True, True)])
def test_plot_polar_heatmap_masking(heatmap_data, mask_angle, mask_radius):
    """Test masking parameters for heatmap."""
    ax = plot_polar_heatmap(
        df=heatmap_data, r_col='value', theta_col='hour',
        mask_angle=mask_angle, mask_radius=mask_radius
    )
    if mask_angle:
...
def test_plot_radial_density_ring_raises_error(heatmap_data):
    ...
    with pytest.raises(ValueError):
        plot_radial_density_ring(df=heatmap_data, kind="direct",
                                 target_cols=["value", "hour"])
    with pytest.raises(ValueError):
        plot_radial_density_ring(df=heatmap_data, kind="width",
                                 target_cols=["value"])
...
def test_plot_polar_quiver_with_color_col(quiver_data):
    ...
    ax = plot_polar_quiver(
        df=quiver_data,
        r_col='radius', theta_col='angle', u_col='radial_change', 
        v_col='tangential_change',
        color_col='magnitude',
